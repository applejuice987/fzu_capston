...
class _Page4State extends State<Page4> {
  ...
  void initState() {
    ...
    if (isInflu) {
      db.collection('userInfoTable').doc('user').collection('user_influencer')
          .doc(_currentUser).get()
          .then((DocumentSnapshot ds) {
        _profileImage = ds['profile'].toString();
      });
    } else {
      db.collection('userInfoTable').doc('user').collection('user_sponsor').doc(
          _currentUser).get().then((DocumentSnapshot ds) {
        _profileImage = ds['profile'].toString();
      });
    }
  }
              ...
              children: [
                SizedBox(
                  ...
                  child: CircleAvatar(
                      ...
                      child: ClipOval(
                          child: _profileImage != '' ? Image.memory(
                              Base64Decoder()
                                  .convert(
                                  _profileImage))
                              : Image.asset(
